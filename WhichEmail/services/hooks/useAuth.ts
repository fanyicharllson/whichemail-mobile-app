...
export const useRegister = () => {
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      showToast.success(
        "Account Created 🎉",
        `Welcome ${data.name} to WhichEmail!`
      );
      try {
...
export const useForgotPassword = () => {
  return useMutation({
    mutationFn: async ({ email }: { email: string }) => {
      // Optional: End any active session
      try {
        await account.deleteSession({ sessionId: "current" });
      } catch {}

      const resetUrl =
        "https://dashboard.charlseempire.tech/apps/whichemail/reset-password";
      return await account.createRecovery({ email: email, url: resetUrl });
    },
    onSuccess: () => {
      showToast.success(
        "Password recovery email sent! ",
        "Check your inbox and tap the link to reset."
      );
    },
    onError: (error: any) => {
      console.error("Failed to send recovery email:", error);
      showToast.error("Failed to send recovery email. Please try again.");
    },
  });
};
